use swc_core::{
  common::DUMMY_SP,
  ecma::{
    ast::{Expr, Id, Ident, JSXElement, JSXElementChild, JSXElementName, JSXExpr, JSXExprContainer, BinExpr, BinaryOp, AssignExpr, AssignOp, PatOrExpr, JSXAttr, JSXAttrName, SpreadElement},
    visit::{noop_visit_type, Visit, VisitWith, VisitMut, noop_visit_mut_type, VisitMutWith}, utils::is_literal,
  }
};
...
pub fn modify_jsx_if_immutable(jsx_ele: &mut JSXElement, state: &mut State) -> bool {
  ...
  let mut self_immutable = match &jsx_ele.opening.name {
    JSXElementName::Ident(ident) => {
      ident.sym.chars().next().unwrap().is_lowercase() || state.vars.contains(&ident.to_id()) || state.config.immutable_globals.contains(&ident.sym.to_string())
    }
      ...
      match c {
        JSXElementChild::JSXElement(ele) => {
          let name = match &ele.opening.name {
            JSXElementName::Ident(id) => format!("_{}", id.sym.to_string()),
            _ => unreachable!(),
          };
